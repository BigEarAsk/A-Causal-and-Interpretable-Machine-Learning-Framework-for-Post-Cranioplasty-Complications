...
def train():
    lr = [0.5,0.1,0.5,0.5,0.5,0.1,0.5,0.5]
    depth = [3,7,5,3,3,5,7,5]
    n = [200,200,100,200,200,200,100,50]
    ...
    for i in range(1):
        ...
        model = XGBClassifier(learning_rate=lr[i],
                                max_depth = depth[i],
                                n_estimators = n[i])
        # train model 
        model.fit(X, y)

        save_model(model,'model_parameters/'+ f'xgboost_{var_name[i]}')
