...
if __name__ == '__main__':
    ...
    data_path = ['置信区间/时序验证-分_标准化后.xlsx']

    names = ['时序验证-分_External Validation']

    prob_standard = {
        'infection':0.605,  # 0.116
        'Fluid':0.569,
        'Penu':0.310, 
        'intra_hem':0.462,  # 0.494
        'Hydro':0.470,
        'Seizures':0.534,
        'total':0.366,
        'Reop':0.673
    ...
    for path,name in zip(data_path,names):
        ...
        for i in range(len(var_name)):
            ...
            model = joblib.load('rf_Total (1).pkl')
            ...
            # compute each value's confidence range
